...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const res = await authApi.register({ name, phone, password, email })
    ...
    <div className="max-w-md mx-auto mt-12">
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
        <div className="flex items-center gap-2 mb-6">
          ...
          <h2 className="text-xl font-bold text-gray-800">商家注册</h2>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm text-gray-600 mb-1">商家名称</label>
            <input
              type="text"
              value={name}
          ...
          <div>
            <label className="block text-sm text-gray-600 mb-1">邮箱（选填）</label>
            <input
              type="email"
              value={email}
